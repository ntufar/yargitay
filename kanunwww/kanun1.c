...
static unsigned long sqlctx = 37283;
...
extern void sqliem(/*_ char *, int * _*/);
 ...
 static char *sq0002 = 
"select kanun ,count(kanun) ,kanunno  from emsal.kanun  group by kanun,kanunn\
o order by kanun            ";

 static char *sq0003 = 
"select kanun ,count(kanun) ,kanunno  from emsal.kanun where daireno=:b0 grou\
p by kanun,kanunno order by kanun            ";
...
{10,4146,0,0,0,
5,0,0,1,0,0,27,162,0,0,4,4,0,1,0,1,9,0,0,1,9,0,0,1,5,0,0,1,10,0,0,
36,0,0,2,104,0,9,192,0,0,0,0,0,1,0,
51,0,0,3,121,0,9,194,0,0,1,1,0,1,0,1,5,0,0,
70,0,0,2,0,0,13,202,0,0,3,0,0,1,0,2,5,0,0,2,3,0,0,2,3,0,0,
97,0,0,3,0,0,13,205,0,0,3,0,0,1,0,2,5,0,0,2,3,0,0,2,3,0,0,
124,0,0,3,0,0,15,221,0,0,0,0,0,1,0,
139,0,0,2,0,0,15,223,0,0,0,0,0,1,0,
};
...
cgiMain()
{
	...
	if(cgiFormGetVal("daire")){
        ...
        cgiPrintf("Bu sayfada kanun listesini görmek için önce ilgili daireyi kutudan seçip"
                " <strong>Ara</strong> tuþuna basýnýz<br>");
        cgiPrintf("Gelen kanunlarý týkladýnýzda maddelere göre týklayarak karara "
                "ulaþabilirsiniz.<br>"
                "Görüþ ve önerileriniz için <a href=\"mailto:teknik@yargitay.gov.tr\">"
                "teknik@yargitay.gov.tr</a><br>");

	cgiPrintf("<FORM method=get ACTION=\"kanun1\">");
	...
	cgiPrintf("<INPUT TYPE=submit VALUE=Gönder>");
	cgiPrintf("</FORM>");

	if(nolist==1)exit(0);
	
	if(!nolist){
		cgiPrintf("<strong>Daire:</strong> %s<br>",textdaire);
	}
...

{
   ...
   sqlcxt((void **)0, &sqlctx, &sqlstm, &sqlfpn);
		...
		cgiPrintf("<TR><TD>%d</TD>"
			"<TD>%d</TD><TD>"
			"<A HREF=\"kanun2?daire=%s&kanun=%s\">"
			"%s</A></TD></TR>",
			count,kanunno,cgiEncodeUrl(daireno),cgiEncodeUrl(kanun),kanun);
	}
	cgiPrintf("</TABLE>");
	if(daireno[0]==0){
		/* EXEC SQL CLOSE kcur; */ 
