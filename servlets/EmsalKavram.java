 ...
 */
public class EmsalKavram extends HttpServlet
{ 
    /**
...
/*------------------------------------------------------------------*/
    ...
    public void doGet (HttpServletRequest request,
		...
		response.setContentType("text/html; charset=iso-8859-9");
            ...
            out.println("<TITLE>Kavram Sorgu Sonucu</TITLE>");
            out.println("</HEAD>");
	...
	out.println("<center><a href=\"http://www.yargitay.gov.tr/\">"+
			...
			"Kavram Sorgu Sonuçlarý</font></h1> </center> <hr>");
		...
		while(values.hasMoreElements()) {
			...
			if(name.compareTo("kavram") == 0){
				kavram = request.getParameterValues("kavram")[0];
			}
		...
		if(adim.compareTo("2") == 0){
			doadim2(out, daire, kavram);
		}
		if(adim.compareTo("3") == 0){
			doadim3(out, daire, esasno);
		}
	...
	private void doadim1(PrintWriter out, String daire){
		String query1 = new String("SELECT kavram, count(kavram) FROM emsal.kavram WHERE daireno=? GROUP BY kavram");
		long ctxid =  99999999;
		...
		out.println("<FORM method=get ACTION=\"EmsalKavram\">");
		...
		try {
			...
			out.println("<TABLE>");
			out.println("<TR><TD><strong>Adet</strong></TD><TD><strong>Kavram</strong></TD></TR>");
			while(rset.next()){
				out.println("<TR><TD>" + rset.getString(2) + "</TD><TD><A HREF=\"EmsalKavram?"+
						"adim=2&daire="	+ java.net.URLEncoder.encode(daire) + 
						"&kavram=" + java.net.URLEncoder.encode(rset.getString(1))+"\">" +
	...
	private void doadim2(PrintWriter out, String daire, String kavram){
		String query1 = new String(iso1to9("SELECT  esasno, daireno FROM emsal.kavram WHERE kavram like ?  and daireno like ?"));
		String query2 = new String("SELECT  esasno, daireno FROM emsal.kavram WHERE kavram like '"+kavram +
						"' and daireno like '"+daire+"%'");
		...
		out.println("<strong>Kavram:</strong> " + iso1to9(kavram) + " <br>");
		...
		query2 = new String("SELECT  esasno, daireno FROM emsal.kavram WHERE kavram = '"+(kavram) + 
						"' and daireno = '"+kisadaire+"'");
		...
		try {
			...
			//rset = kavramlar.executeQuery();
			kavramlar2 = conn.createStatement();
			rset = kavramlar2.executeQuery(iso1to9(query2));
			out.println("<TABLE>");
			out.println("<TR><TD><strong>Daire</strong></TD><TD><strong>Esas sýra numarasý</strong></TD></TR>");

			while(rset.next()){
				out.println("<TR><TD>" + daire + "</TD><TD>" +
					"<A HREF=\"EmsalKavram?adim=3&daire=" + java.net.URLEncoder.encode(daire) +
...
/*
	private void doadim3(PrintWriter out, String daire, String esasno){
		Statement stmt = null;
		ResultSet rset = null;
		String b = new String("");
		Long lesno = new Long(esasno);
		String kisadaire = null;
		...
		try{
			...
			ostmt.close();
            		...
            		while(e != null) {
				log("adim3: SQLException: " + e.getSQLState() + "(1)" +
		    			e.getMessage() + "(2)" +
		    			e.getErrorCode() + "(3)");
					e = e.getNextException();
            		}
